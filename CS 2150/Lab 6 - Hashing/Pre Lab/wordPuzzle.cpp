...
int main(int argc , char **argv){
  ...
  //cout << numwords << endl;
  //cout << (checkprime(numwords / .75) ? (numwords / .75) : getNextPrime(numwords / .75)) << endl;
  myhash = HashTable(checkprime(numwords / .75) ? (numwords / .75) : getNextPrime(numwords / .75));
  
  while ( getline (count,line) )
  {
    if(line.size() > maxlength)
      maxlength = line.size();
      HashNode *temp = new HashNode(line,hash(line,numwords));
      myhash.insert(temp);
      //cout << line << '\n';
   }
   count.close();
   //cout << maxlength << endl;
  //for(int i = 0; i < (checkprime(numwords / .75) ? (numwords / .75) : getNextPrime(numwords / .75)); i ++)
    // cout << myhash.getValue(i) << endl;
   ...
   for (int i = 0; i < rows; i++)
	 ...
	 for (int l = 3; l <= maxlength; l++)
	 {
	   ...
	   if(found.count(str) == 0 && myhash.find(hash(str,numwords),str))
...
int hash(string s,int size){
  return (int(s[0]) * int(s[1]) * s.size()) % size;
}
